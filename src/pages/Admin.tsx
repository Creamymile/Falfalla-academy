import {
  ...
  Image, KeyRound, Languages, Loader, MessageCircle, Play, Plus, RotateCcw, Search, Settings,
  Trash2, Upload, Users, Video, BarChart3, GraduationCap, Wrench,
} from "lucide-react";
...
export function Admin({
  ...
  const importContent = (file?: File) => {
    ...
    reader.onload = () => {
      try {
        ...
        setImportMessage("Course content imported successfully.");
      } catch {
  ...
  const tabs: { id: Tab; label: string; icon: React.ReactNode }[] = [
    ...
    { id: "codes", label: "Access Codes", icon: <KeyRound size={16} /> },
    { id: "students", label: "Students", icon: <Users size={16} /> },
    { id: "requests", label: "Requests", icon: <MessageCircle size={16} /> },
  ];
    ...
    <section className="page admin-page">
      ...
      <div className="admin-tabs">
        {tabs.map((t) => (
          ...
          >
            {t.icon} {t.label}
            {t.id === "requests" && requests.filter((r) => r.status === "new").length > 0 && (
              <span className="tab-badge">{requests.filter((r) => r.status === "new").length}</span>
            )}
          </button>
        ))}
      </div>
      ...
      {tab === "overview" && (
        <div className="admin-tab-content">
          ...
          <h2 className="admin-section-title">Content readiness</h2>
          ...
          <section className="editor-panel">
            ...
            <div className="admin-quick-actions">
              <button className="secondary" onClick={exportContent}><FileDown size={16} /> Export courses</button>
              <label className="secondary file-button"><FileUp size={16} /> Import courses<input type="file" accept="application/json,.json" onChange={(e) => importContent(e.target.files?.[0])} /></label>
              <button className="secondary" onClick={() => updateCourses(seedCourses)}><RotateCcw size={16} /> Reset to defaults</button>
            </div>
        ...
        <div className="admin-tab-content">
          ...
          <div className="admin-course-bar">
            <div className="admin-course-selector">
              {courses.map((c) => (
                <button
                  className={`admin-course-chip ${c.id === selectedCourse.id ? "active" : ""}`}
                  key={c.id}
                ...
                >
                  ...
                  <span>
                    ...
                    <small>{c.modules.length} modules, {courseLessonCount(c)} lessons</small>
                  </span>
                </button>
              ))}
            </div>
          ...
          <section className="editor-panel">
            ...
            <div className="editor-grid-2col">
            ...
            <label>Learning outcomes <small style={{ fontWeight: 400, textTransform: "none" }}>(one per line)</small><textarea value={selectedCourse.outcomes.join("\n")} onChange={(e) => patchCourse(selectedCourse.id, { outcomes: e.target.value.split("\n").map((s) => s.trim()).filter(Boolean) })} /></label>
          </section>
          ...
          <section className="editor-panel">
            ...
            {selectedModule && (
              <>
                ...
                <button className="danger small" onClick={() => deleteModule(selectedModule.id)} disabled={selectedCourse.modules.length <= 1}><Trash2 size={14} /> Delete module</button>
              </>
            )}
          </section>
          ...
          <section className="editor-panel">
            ...
            {selectedLesson ? (
              <>
                ...
                <label>Practice drills <small style={{ fontWeight: 400, textTransform: "none" }}>(one per line)</small><textarea value={selectedLesson.practice.join("\n")} onChange={(e) => patchLesson(selectedCourse.id, selectedLesson.id, { practice: e.target.value.split("\n").map((s) => s.trim()).filter(Boolean) })} /></label>
                ...
                <div className="resource-editor">
                  <div className="editor-heading"><h3>Subtitle tracks</h3><button className="secondary small" onClick={addSubtitle}><Plus size={14} /> Add subtitle</button></div>
                  {selectedLesson.subtitles.map((sub) => (
                    <div className="subtitle-edit-row" key={sub.id}>
                      ...
                      <input value={sub.src} onChange={(e) => patchLesson(selectedCourse.id, selectedLesson.id, { subtitles: selectedLesson.subtitles.map((s) => s.id === sub.id ? { ...s, src: e.target.value } : s) })} placeholder="VTT URL" />
                      <div className="subtitle-edit-actions">
                        <input type="file" accept=".vtt" onChange={(e) => handleSubtitleUpload(sub.id, e.target.files?.[0])} />
                        <button className="danger small" onClick={() => deleteSubtitle(sub.id)}><Trash2 size={14} /></button>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                {/* Resources */}
                <div className="resource-editor">
                  <div className="editor-heading"><h3>Downloadable materials</h3><button className="secondary small" onClick={addResource}><Plus size={14} /> Add material</button></div>
                  {selectedLesson.resources.map((res) => (
                    <div className="resource-edit-row" key={res.id}>
                      <input value={res.title} onChange={(e) => patchLesson(selectedCourse.id, selectedLesson.id, { resources: selectedLesson.resources.map((r) => r.id === res.id ? { ...r, title: e.target.value } : r) })} placeholder="Material title" />
                      <input value={res.url} onChange={(e) => patchLesson(selectedCourse.id, selectedLesson.id, { resources: selectedLesson.resources.map((r) => r.id === res.id ? { ...r, url: e.target.value } : r) })} placeholder="URL" />
                      ...
                      <div className="resource-edit-actions">
                        ...
                        <button className="danger small" onClick={() => deleteResource(res.id)}><Trash2 size={14} /></button>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            ) : <p className="muted-copy">Add a lesson to start editing materials.</p>}
          </section>
        </div>
      )}
        ...
        <div className="admin-tab-content">
          <section className="editor-panel">
            ...
            <div className="access-code-generator">
              ...
              <label>
                Duration (days)
                <input type="number" min={1} max={365} value={newCodeDays} onChange={(e) => setNewCodeDays(Number(e.target.value) || 7)} />
              </label>
              <button className="primary" disabled={codeLoading} onClick={async () => {
                setCodeLoading(true);
                const result = await createAccessCodeInDb(newCodeCourseId, newCodeDays);
                if (result.ok) setAccessCodes([result.accessCode, ...accessCodes]);
                setCodeLoading(false);
              }}>
                {codeLoading ? <><Loader size={16} className="spin" /> Generating...</> : <><Plus size={16} /> Generate code</>}
              </button>
            </div>
          </section>
                        ...
                        <tr key={ac.id}>
                          ...
                          <td>
                            <button
                              ...
                              onClick={async () => {
                                await toggleAccessCodeInDb(ac.id, !ac.isActive);
                                setAccessCodes(accessCodes.map((c) => c.id === ac.id ? { ...c, isActive: !c.isActive } : c));
                              }}
                            >
      ...
      {tab === "requests" && (
        <div className="admin-tab-content">
          <section className="editor-panel">
            <div className="editor-heading">
              <h2>Student lesson requests</h2>
              <span className="admin-mode">{requests.length} total</span>
            </div>
            {requests.length ? (
                  ...
                  return (
                    <article key={r.id} className="request-admin-card">
                      ...
                      <div className="request-card-actions">
                        ...
                        <button className="danger small" onClick={() => updateRequests(requests.filter((x) => x.id !== r.id))}><Trash2 size={14} /> Delete</button>
                      </div>
                    </article>
                  );
                })}
              ...
              <div className="empty-state">
                ...
                <p>Student lesson requests will appear here once students start submitting them.</p>
              </div>
            )}
          </section>
        </div>
